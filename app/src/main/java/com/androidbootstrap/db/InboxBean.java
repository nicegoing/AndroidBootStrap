...
public abstract class InboxBean implements InboxBeanModel {
    ...
    public static final  Factory<InboxBean> FACTORY      = new Factory<>(new Creator<InboxBean>() {
        @Override
        public InboxBean create(long _id, @NonNull String title, @NonNull String expected_result, @NonNull String todo, @NonNull Calendar add_time, @NonNull String type) {
            return new AutoValue_InboxBean(_id, title, expected_result, todo, add_time, type);
        }
    }, DATE_ADAPTER);
